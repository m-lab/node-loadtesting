...
for ws_proto in $WS_PROTOS; do
  ...
  AVERAGE_C2S=$(grep 'Measured upload rate' * | awk '{count++; sum += $5;} \
    END {print (sum/count)/1000}')
  AVERAGE_S2C=$(grep 'Measured download rate' * | awk '{count++; sum += $5;} \
    END {print (sum/count)/1000}')
  ...
  if [ "$TOTAL_FAILURES" -gt 0 ]; then
    ...
    DIED_C2S_PREPARE=$(grep -B 5 'Ran for N seconds' $FAILED_TESTS \
      | grep 'C2S type 3' | wc -l)
    DIED_C2S_START=$(grep -B 5 'Ran for N seconds' $FAILED_TESTS \
      | grep 'C2S type 4' | wc -l)
    DIED_S2C_PREPARE=$(grep -B 5 'Ran for N seconds' $FAILED_TESTS \
      | grep 'CALLED S2C with 3' | wc -l)
    DIED_S2C_START=$(grep -B 5 'Ran for N seconds' $FAILED_TESTS \
      | grep 'CALLED S2C with 4' | wc -l)
...
Protocol: $ws_proto
  ...
  Percent failed tests: $PERCENT_FAILED%  \
    ($TOTAL_FAILURES out of $TESTS_NOT_KILLED_COUNT)
...
pushd $CLIENT_LOGS/$C_CLIENT > /dev/null
  ...
  TESTS_NOT_KILLED=$(grep -L 'Exited with code 137' *)
  TESTS_NOT_KILLED_COUNT=$(grep -L 'Exited with code 137' * | wc -l)
  TESTS_GOT_SRV_QUEUE=$(grep -l 'Another client is currently begin served' * \
    | wc -l)
  FAILED_TESTS=$(grep -L 'Exited with code 0' $TESTS_NOT_KILLED)
  TOTAL_FAILURES=$(echo $FAILED_TESTS | wc -w)
  AVERAGE_C2S=$(for log in *; do awk '/[0-9]+\.[0-9]+ Mb\/s/{i++; if (i==1) \
    print $1 }' $log; done | awk '{count++; sum += $1} END {print sum/count}')
  AVERAGE_S2C=$(for log in *; do awk '/[0-9]+\.[0-9]+ Mb\/s/{i++; if (i==2) \
    print $1 }' $log; done | awk '{count++; sum += $1} END {print sum/count}')
  if [ "$TOTAL_FAILURES" -gt 0 ]; then
    ZERO_SECS=$(grep 'Ran for N seconds' $FAILED_TESTS \
      | awk '{if ($5 == 0) print}' | wc -l)
    DIED_C2S=$(grep '^running 10\.0s outbound test.*Exited with code 137$' \
      $FAILED_TESTS | wc -l)
    DIED_S2C=$(grep '^running 10\.0s inbound test.*Exited with code 137$' \
      $FAILED_TESTS | wc -l)
    DIED_CLIENT_SOCK=$(grep -B 1 'Exited with code 137' $FAILED_TESTS \
